...
public class StageApprovalModule extends FormModule{
	public JPanel createTab(JDialog editor,Editable editable,boolean isNew,Runnable deleter){
		...
		if(User.getActiveUser().hasPermission(p.getStage().approver))ok.addActionListener(evt->{
			...
			JDialog d=new JDialog(editor,true);
			...
			B approve=new B();
			approve.addActionListener(e->{p.approve(comment.getText());d.dispose();});
			...
			if(User.getActiveUser().hasPermission(p.getStage().rejecter)){
				B reject=new B(){};
				reject.addActionListener(e->{p.reject(comment.getText());d.dispose();});
