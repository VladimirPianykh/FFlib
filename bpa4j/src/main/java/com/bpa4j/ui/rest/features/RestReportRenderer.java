...
public class RestReportRenderer implements ReportRenderer {
	...
	public void render(FeatureRenderingContext ctx) {
		...
		int contentHeight = configPanel != null ? 
			root.getHeight() - header.getHeight() - configPanel.getHeight() : 
			root.getHeight() - header.getHeight();
		...
		if (rendererCount > 0) {
			...
			for (Report.DataRenderer r : dataRenderers) {
				...
				// TODO: Create REST-specific components for data renderers
