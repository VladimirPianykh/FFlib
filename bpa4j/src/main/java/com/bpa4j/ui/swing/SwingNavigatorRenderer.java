...
public class SwingNavigatorRenderer extends JDialog implements NavigatorRenderer{
    public void render(Navigator navigator){
		...
		class L extends HButton{
			public L(ImplementedInfo info){
				super(10,5);
				addActionListener(e->HelpView.show(info));
				setFont(font);
		...
		for(ImplementedInfo path:HelpView.paths)p.add(new L(path));
    ...
    public void renderInfo(ImplementedInfo info){
        new HelpViewDialog(Window.getWindows()[0],info);
    }
    public void init(){
        Toolkit.getDefaultToolkit().addAWTEventListener(new AWTEventListener(){
			public void eventDispatched(AWTEvent e){
				if(e.getID()==KeyEvent.KEY_RELEASED&&(((KeyEvent)e).getModifiersEx()&KeyEvent.ALT_DOWN_MASK)!=0&&((KeyEvent)e).getKeyCode()==KeyEvent.VK_H)new Navigator();
			}
