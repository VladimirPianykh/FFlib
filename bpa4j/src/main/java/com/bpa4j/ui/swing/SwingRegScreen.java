...
import com.bpa4j.ui.swing.util.Switcher;
import com.formdev.flatlaf.FlatDarkLaf;

public class SwingRegScreen implements RegScreen{
	public void show(ProgramStarter.StarterContext context){
		...
		JTextArea a=new JTextArea(context.getWelcomeMessage());
		...
		if(context.isAuthRequired()){
			...
			AbstractAction action=new AbstractAction(){
				public void actionPerformed(ActionEvent e){
					...
					int result=context.login(log.getText(),pass.getText(),reg.on);
					if(result==0)new Message(reg.on?"Пользователь зарегистрирован.":"Вход выполнен.",Color.GREEN);
					else if(result==1)new Message(reg.on?"Аккаунт уже существует.":"Неизвестный пользователь.",Color.RED);
					else if(result==2)new Message("Неверный пароль.",Color.RED);
					if(result==0)f.dispose();
				}
			};
			...
			AbstractAction action=new AbstractAction(){
				public void actionPerformed(ActionEvent e){
					context.enterWithoutAuth((User)c.getSelectedItem());
					f.dispose();
				}
			};
