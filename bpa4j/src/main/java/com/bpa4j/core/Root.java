...
public final class Root{
	...
	public static ImageIcon loadIcon(String path){
		if(!imageResources.containsKey(path))imageResources.put(path,new ImageIcon(Objects.requireNonNull(getResource("resources/"+path))));
		return imageResources.get(path);
	}
	public static ImageIcon loadIcon(String path,int w,int h){
		String p=path+":"+w+"x"+h;
		if(!imageResources.containsKey(p))imageResources.put(p,new ImageIcon(new ImageIcon(Objects.requireNonNull(getResource("resources/"+path))).getImage().getScaledInstance(w,h,0)));
